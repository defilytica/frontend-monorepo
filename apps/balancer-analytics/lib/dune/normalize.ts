...
const CHAIN_MAP: Record<string, GqlChain> = {
  ...
  avalanche_c: GqlChain.Avalanche,
  avalanche:   GqlChain.Avalanche,
  zkevm:       GqlChain.Zkevm,
  monad:       GqlChain.Monad,
  fantom:      GqlChain.Fantom,
...
export function nameToCategory(name: string): SourceCategory {
  ...
  // Bridges
  const BRIDGE_NAMES = [
    'layerzero', 'stargate', 'across', 'wormhole', 'synapse', 'hop', 'celer',
    'chainhop', 'interport', 'mux', 'gravity', 'multichain', 'relay', 'squid',
    'debridge', 'bungee', 'socket',
  ]
...
export function normalizeDuneRow(raw: {
  ...
  if (typeof raw.address !== 'string' || typeof raw.name !== 'string' || typeof raw.blockchain !== 'string') {
    return null
  }
